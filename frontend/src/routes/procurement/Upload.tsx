...
export default function UploadContract() {
  ...
  async function handleSubmitToLegal() {
    ...
    await maybeUpload(data.id as string)
    alert('Kontrak dikirim ke Legal')
  }
    ...
    <div className="space-y-6">
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="space-y-6">
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <header className="space-y-1">
              <h1 className="text-2xl font-semibold text-slate-900">Upload Contract</h1>
              <p className="text-sm text-slate-500">Unggah dokumen kontrak baru untuk dianalisis oleh sistem AI.</p>
            </header>
            ...
            <div
              ...
              className={`mt-6 flex cursor-pointer flex-col items-center justify-center rounded-3xl border-2 border-dashed px-6 py-12 text-center transition ${
                dragging ? 'border-[#357ABD] bg-[#357ABD]/5' : 'border-slate-300 bg-slate-50'
              }`}
            >
              ...
              <p className="mt-4 text-sm font-semibold text-slate-700">Tarik dan letakkan file kontrak di sini</p>
              <p className="text-xs text-slate-500">atau klik untuk unggah file (PDF, DOCX, TXT)</p>
              <button
                type="button"
              ...
              />
              {file && <div className="mt-3 text-xs text-slate-500">File terpilih: {file.name}</div>}
            </div>
          </section>

          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <header className="space-y-1">
              <h2 className="text-lg font-semibold text-slate-900">Informasi Kontrak (Opsional)</h2>
              <p className="text-sm text-slate-500">Lengkapi detail singkat untuk membantu tim legal melakukan identifikasi.</p>
            </header>
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              ...
              <Field label="Jenis Kontrak">
                <select
                  ...
                  value={meta.type}
                  onChange={(event) => setMeta((prev) => ({ ...prev, type: event.target.value }))}
                >
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <button
                type="button"
                className="inline-flex items-center rounded-xl bg-[#357ABD] px-5 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-[#2e6dad]"
              ...
              <button
                type="button"
                className="inline-flex items-center rounded-xl border border-slate-300 px-5 py-2 text-sm font-semibold text-slate-600 shadow-sm transition hover:bg-slate-50"
        ...
        <aside className="space-y-6">
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <h3 className="text-sm font-semibold text-slate-800">Status Kontrak Terbaru</h3>
            <div className="mt-4 space-y-4">
              {latestContracts.length === 0 && (
                <p className="text-xs text-slate-500">Belum ada kontrak terbaru.</p>
              )}
              {latestContracts.map((contract, index) => (
                <LatestStatusCard key={contract.id} contract={contract} highlight={index === 0} />
              ))}
            </div>
          </section>
...
function LatestStatusCard({ contract, highlight }: { contract: ContractRow; highlight: boolean }) {
  const badgeClass = STATUS_BADGE[contract.status] ?? 'bg-slate-100 text-slate-600 border border-slate-200'
  const riskClass = contract.risk ? RISK_BADGE[contract.risk] ?? 'bg-slate-100 text-slate-600 border border-slate-200' : null

  return (
    <div className={`space-y-3 rounded-2xl border px-4 py-4 ${highlight ? 'border-[#357ABD]/40 bg-[#357ABD]/5' : 'border-slate-200 bg-slate-50'}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="space-y-1">
          <div className="text-sm font-semibold text-slate-800">{contract.name || 'Untitled Contract'}</div>
          <div className="text-xs text-slate-500">{statusMessage(contract.status)}</div>
        </div>
      ...
      <div className="flex items-center gap-2 text-xs text-slate-500">
        <span>Diperbarui {formatRelativeTime(contract.updated_at ?? contract.created_at)}</span>
        {riskClass && (
          <span className={`inline-flex items-center rounded-full px-2 py-1 text-[10px] font-semibold ${riskClass}`}>
            {contract.risk}
          </span>
        )}
      </div>
